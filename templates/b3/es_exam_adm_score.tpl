...
<form>
  <select    class="form-control"  onChange="window.location.href='score.php?assn='+this.value" >
    ...
    <{foreach from=$exam_list  item=exam_list}>
        <option value='<{$exam_list.assn}>' <{if $assn==$exam_list.assn}>selected<{/if}>> <{$class_list_c[$exam_list.class_id]}>  -- <{$exam_list.title}> (<{$exam_list.uid_name}>) </option>
      <{/foreach}>
...
<{if ($assn) }>
  <div class="row">
    <div class="col-md-5 col-xs-5"><h3><{$class_list_c[$data.exam.class_id]}>   <{$data.exam.title}></h3></div>
  ...
  <th class="col-md-1 col-xs-2"><{$smarty.const._TAD_FUNCTION}></th>
    </tr>
    </thead>
  ...
  <{/if}>
      ...
      <td class="col-md-1 col-xs-2"><{$all.author}><{if $class_students[$all.sit_id].team_sitid_list_name}> 組員：<{$class_students[$all.sit_id].team_sitid_list_rname}><{/if}> </td>
  ...
  <{if (!$stud.in)  }>
 	<span class="col-md-2 col-xs-2"><span class="badge badge-info"><{ $sit }></span> <{ $stud.name }>  </span>
  <{/if}>
   <{/foreach}>
</div>
